...
import type { ErrorResponse, Id, Params, SuccessResponse } from "@valkyr/json-rpc";

import { isJsonRpcId } from "./utils.ts";

const RECONNECT_INCREMENT = 1250; // 1.25 seconds
const MAX_RECONNECT_DELAY = 1000 * 30; // 30 seconds

/**
 * Provides a request response instance for JSON-RPC 2.0 compliant communiaction
 * between client and server over a WebSocket connection.
 */
    ...
    if (ev.code !== 4000) {
      ...
      this.#debounce.reconnect = setTimeout(
        ...
        this.#reconnectDelay < MAX_RECONNECT_DELAY
          ? (this.#reconnectDelay += RECONNECT_INCREMENT)
          : MAX_RECONNECT_DELAY,
      );
    }
  }
